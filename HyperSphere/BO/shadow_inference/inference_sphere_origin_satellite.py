...
class ShadowInference(Inference):
	...
	def predict(self, pred_x, hyper=None, stability_check=False):
		...
		# K_ori_sat_diag_product_radius = self.model.kernel.product_kernel_radius(one_radius * 0, one_radius * n_dim ** 0.5).repeat(n_pred, 1)
		...
		#                                                   prod_gram_radial=K_ori_sat_diag_product_radius)
		K_ori_sat_diag = self.model.kernel.combine_kernel(radial_gram=K_ori_sat_diag_radius, sphere_gram=self.model.kernel.radius_kernel.forward_on_identical())
		K_sat_pre_diag_radius = self.model.kernel.radius_kernel(pred_x_radius, one_radius * n_dim ** 0.5)
		# K_sat_pre_diag_product_radius = self.model.kernel.product_kernel_radius(pred_x_radius, one_radius * n_dim ** 0.5)
		...
		#                                                   prod_gram_radial=K_sat_pre_diag_product_radius)
		K_sat_pre_diag = self.model.kernel.combine_kernel(radial_gram=K_sat_pre_diag_radius, sphere_gram=self.model.kernel.radius_kernel.forward_on_identical())
