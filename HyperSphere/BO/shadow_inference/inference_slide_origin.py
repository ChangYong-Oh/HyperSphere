...
class ShadowInference(Inference):
	...
	def predict(self, pred_x, hyper=None):
		...
		y_vec = torch.mm(mean_vec_other.t(), Ainv_B).view(-1, 1) - torch.mean(mean_vec_origin)

		slide_origin_mean_adjustment = (identity_const + onemat_const * n_train_origin) * k_vec * y_vec * n_train_origin
		slide_origin_quad_adjustment = (identity_const + onemat_const * n_train_origin) * k_vec ** 2 * n_train_origin
...
if __name__ == '__main__':
	...
	if ndim == 2:
		...
		# 	inference_input_map = Inference((torch.cat([slide_origin[i:i + 1], x_input_input_map], 0), output), model_input_map)
		# 	mean_input_map, var_input_map = inference_input_map.predict(x_pred_points_input_map[i:i + 1])
