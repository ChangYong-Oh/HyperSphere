...
def spherical_BO(func, n_eval=200):
	...
	rectangular_input = Variable(torch.ger(torch.arange(0, ndim), torch.FloatTensor([0, min(search_sphere_radius / ndim ** 0.5, search_cube_half_sidelength)])))
	output = Variable(torch.zeros(rectangular_input.size(0), 1))
	for i in range(rectangular_input.size(0)):
		output[i] = func(rectangular_input[i])
	spherical_input = rect2spherical(rectangular_input)
	spherical_input[spherical_input != spherical_input] = 0
