...
class RadializationWarpingKernel(GPModule):
	...
	def __init__(self, max_power, search_radius):
		...
		self.radius_kernel = Matern52(ndim=1, input_map=input_warping(ndim=1, max_input=search_radius), max_ls=search_radius * 0.5, trainable_amp=False)
		self.product_kernel_radius = Matern52(ndim=1, input_map=input_warping(ndim=1, max_input=search_radius), max_ls=search_radius * 0.5, trainable_amp=False)
	...
	def init_parameters(self, amp):
		...
		self.radius_kernel.log_ls.data.fill_(self.search_radius * 0.25).log_()
		self.product_kernel_radius.log_ls.data.fill_(self.search_radius * 0.25).log_()
