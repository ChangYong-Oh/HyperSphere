...
class RadializationKernel(GPModule):
	...
	def __init__(self, max_power, search_radius):
		...
		self.radius_kernel = Matern52(ndim=1, max_ls=2.0 * search_radius, trainable_amp=False)
	...
	def kernel_amp(self):
		return torch.sum(torch.exp(torch.cat([self.log_amp_prod, self.log_amp_sphere, self.log_amp_radius])))
	...
	def combine_kernel(self, radial_gram, sphere_gram, prod_gram_radial=None, prod_gram_sphere=None):
		...
		amp_radius = torch.exp(self.log_amp_radius)
		amp_sphere = torch.exp(self.log_amp_sphere)
		return prod_gram_radial * prod_gram_sphere * amp_prod + radial_gram * amp_radius + sphere_gram * amp_sphere
